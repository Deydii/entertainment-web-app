...
const TvSeries: NextPageWithLayout = () => {
  ...
  const series:Results[] = results.filter(result => result.title?.toLowerCase().includes(show.toLowerCase()) || result.name?.toLowerCase().includes(show.toLowerCase()));
  ...
  {show && (
    <>
      <h3 className="mt-4 text-[20px] text-white md:text-2xl">{`Found ${series.length} results for ${show}`}</h3>
        <div className="mr-4 md:mr-6 lg:mr-8 lg:mt-8 grid grid-cols-1 gap-x-4 md:gap-x-7 lg:gap-x-10 gap-y-8 min-[375px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 min-[1700px]:grid-cols-5">
          {series.map(({ id, name, title, first_air_date, release_date, backdrop_path, media, isBookmarked }) => {
            return (
              <Card 
                key={id}
                id={id}
